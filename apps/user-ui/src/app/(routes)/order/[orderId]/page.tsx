...
const Page = () => {
  ...
  const badgeClass = STATUS_BADGE[paymentStatusKey] || "bg-gray-100 text-gray-600 border-gray-200";
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border capitalize ${badgeClass}`}>
          {order.status}
        </span>
      </div>
              ...
              <div key={step} className="flex-1 flex items-center">
                <div className="flex flex-col items-center gap-1.5 relative">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                    isCurrent ? "border-blue-500 bg-blue-500 shadow-md shadow-blue-200"
                  ...
                  <span className={`text-[10px] font-medium whitespace-nowrap absolute top-7 ${
                    isCurrent ? "text-blue-600" : reached ? "text-green-600" : "text-gray-400"
                  }`}>
                    {step}
                  </span>
                </div>
                {idx !== DELIVERY_STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-1 ${reached ? "bg-green-400" : "bg-gray-200"}`} />
                )}
              </div>
            );
          })}
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-8">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-2 text-sm text-gray-700">
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-semibold capitalize px-2 py-0.5 rounded-full text-xs border ${badgeClass}`}>{order.status}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium capitalize">{(order.fulfillmentType || "delivery").replace(/_/g, " ")}</span>
          </div>
          <div className="flex justify-between">
          ...
          {order.estimatedDeliveryFee > 0 && (
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {order.isHarareDelivery ? "~" : ""}${order.estimatedDeliveryFee.toFixed(2)}
                {order.isHarareDelivery && <span className="text-xs text-gray-400 ml-1">(estimate)</span>}
              </span>
            </div>
          )}
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm text-sm text-gray-700">
          {order.fulfillmentType === "collection" && order.collectionPoint ? (
            <>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Collection Point</p>
              <p className="font-semibold text-gray-800">{order.collectionPoint.name}</p>
              <p className="text-gray-500 mt-1">{order.collectionPoint.address}</p>
              <p className="text-xs text-green-600 font-medium mt-2">✓ Free collection</p>
            </>
            ...
            <>
              ...
              <p className="text-gray-500">{order.shippingAddress.city}, {order.shippingAddress.zip}</p>
              <p className="text-gray-500">{order.shippingAddress.country}</p>
              {order.isHarareDelivery && (
                <p className="text-xs text-blue-500 mt-2">📍 Exact delivery fee confirmed via WhatsApp</p>
              )}
            </>
          ) : null}
        </div>
      ...
      {order.status === "pending" && (
        <div className="mb-6 flex items-start gap-3 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 text-sm text-yellow-800">
          ...
          <span>Your order has been received and is awaiting confirmation. We'll reach out on WhatsApp shortly.</span>
        </div>
      )}
      ...
      <div>
        ...
        <div className="space-y-3">
          {order.items.map((item: any) => (
            <div key={item.productId} className="border border-gray-100 rounded-2xl p-4 flex items-center gap-4 shadow-sm bg-white">
              <img
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-800 truncate">{item.product?.title || "Unnamed Product"}</p>
              ...
              <p className="text-sm font-bold text-gray-800 flex-shrink-0">${item.price.toFixed(2)}</p>
            </div>
          ))}
        </div>
      </div>
