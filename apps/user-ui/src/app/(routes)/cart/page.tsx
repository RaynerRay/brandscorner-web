...
import {
  ...
  Package, Plus, ShoppingBag, Store, Truck, X,
} from "lucide-react";
...
function getDeliveryFeeLabel(city: string): { label: string; price: number | null; isHarare: boolean } {
  if (!city) return { label: "", price: null, isHarare: false };
  const normalized = city.trim().toLowerCase();
  if (normalized === "harare") return { label: "Calculated by distance", price: null, isHarare: true };
  const match = deliveryFees.cities.find((c: any) => c.name.toLowerCase() === normalized);
  if (match) return { label: `$${match.price.toFixed(2)}`, price: match.price, isHarare: false };
  const special = deliveryFees.specialCities?.find((c: any) => c.name.toLowerCase() === normalized);
  if (special) return { label: `$${special.price.toFixed(2)}`, price: special.price, isHarare: false };
...
function getHarareEstimate(rangeIndex: number): number {
  ...
  if (rangeIndex < 0 || rangeIndex >= ranges.length) return ranges[ranges.length - 1].price;
...
const WhatsAppCheckoutModal = ({
  cart, subtotal, discountAmount, discountPercent,
  discountedProductId, storedCouponCode, selectedAddress, onClose, onOrderPlaced,
}: ModalProps) => {
  ...
  const { label: feeLabel, price: feePrice, isHarare } = getDeliveryFeeLabel(deliveryCity);
  const estimatedDeliveryFee = isHarare
    ? getHarareEstimate(harareRangeIndex)
    : (feePrice ?? 0);
  const orderTotal = fulfillment === "delivery" ? total + estimatedDeliveryFee : total;
  ...
  const buildMessage = () => {
    ...
    cart.forEach((item, i) => {
      ...
      if (item.selectedOptions?.color) lines.push(`   🎨 Color: ${hexToColorName(item.selectedOptions.color)}`);
      if (item.selectedOptions?.size)  lines.push(`   📐 Size: ${item.selectedOptions.size}`);
      lines.push(
    ...
    lines.push("━━━━━━━━━━━━━━━━━━━━━");
    if (discountAmount > 0) lines.push(`🏷️ Coupon *${storedCouponCode}*: -$${discountAmount.toFixed(2)}`);
    ...
    if (fulfillment === "delivery") {
      ...
      if (isHarare) {
        ...
        lines.push(`📍 Estimated distance: ${range.min}–${range.max} km from CBD`);
        lines.push(`🚗 Estimated delivery fee: ~$${estimatedDeliveryFee.toFixed(2)}`);
        lines.push(`_I will share my exact location so you can confirm the final delivery fee._`);
      } else if (feePrice) {
        lines.push(`🚗 Delivery fee: $${feePrice.toFixed(2)}`);
      }
      lines.push(`💰 *Order Total (incl. delivery): ~$${orderTotal.toFixed(2)}*`);
    } else {
    ...
    lines.push("━━━━━━━━━━━━━━━━━━━━━");
    lines.push(`💳 *Payment:* ${paymentMethod === "cash_on_delivery" ? "Cash on Delivery 💵" : "Mobile Payment 📱"}`);
    lines.push("━━━━━━━━━━━━━━━━━━━━━");
  ...
  const handleSendOrder = async () => {
    ...
    try {
      const collectionPoint = fulfillment === "collection"
      ...
      const res = await axiosInstance.post("/order/api/create-order", {
        ...
        ...(fulfillment === "delivery" && {
          ...
          estimatedDeliveryFee: isHarare ? estimatedDeliveryFee : (feePrice ?? 0),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={!sent ? onClose : undefined} />
        ...
        <div className="overflow-y-auto flex-1">
                ...
                <div key={item.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">
                      Qty {item.quantity}{item.selectedOptions?.size ? ` · ${item.selectedOptions.size}` : ""}
                    </p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    {isDiscounted && <p className="text-xs text-gray-400 line-through leading-none">${(item.sale_price * item.quantity).toFixed(2)}</p>}
                    <p className={`text-sm font-semibold ${isDiscounted ? "text-green-600" : "text-gray-800"}`}>
          ...
          {!sent && (
            <div className="px-6 py-4 space-y-5">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-2">How would you like to receive your order?</p>
                  ...
                  {selectedAddress ? (
                    <div className="flex items-start gap-2 text-sm text-gray-700 bg-gray-50 rounded-xl p-3 border border-gray-200">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-xs">{selectedAddress.street}, {selectedAddress.city}, {selectedAddress.country}</p>
                      </div>
                    </div>
                  ) : (
                    ...
                    <div className="rounded-xl border border-gray-200 overflow-hidden">
                      <div className="flex items-center justify-between px-3 py-2.5 bg-gray-50">
                        <div className="flex items-center gap-1.5 text-sm text-gray-600">
                          ...
                          <span>Delivery to <strong>{selectedAddress.city}</strong></span>
                        </div>
                        <span className={`text-sm font-semibold ${isHarare ? "text-blue-600" : "text-green-700"}`}>
                          {isHarare ? `~$${estimatedDeliveryFee.toFixed(2)}` : feeLabel}
                        </span>
                      </div>
                      ...
                      {isHarare && (
                        <div className="px-3 py-3 space-y-2 bg-blue-50/50 border-t border-blue-100">
                          <p className="text-xs text-blue-700 font-medium">
                            📍 Harare delivery is distance-based. Select your approximate distance from CBD:
                          </p>
                          <div className="grid grid-cols-3 gap-1.5">
                            {deliveryFees.harare.ranges.map((range: any, idx: number) => (
                              <button
                                key={idx}
                          ...
                          <p className="text-xs text-blue-600 mt-1">
                            💬 You'll share your exact location on WhatsApp so we can confirm the final fee.
                          </p>
                        </div>
                      )}
                      ...
                      {!isHarare && !feePrice && selectedAddress.city && (
                        <div className="px-3 py-2 border-t border-gray-100 text-xs text-gray-500">
                          We'll confirm the delivery fee for your area on WhatsApp.
                        </div>
                      )}
                    </div>
                  )}
              ...
              {fulfillment === "collection" && (
                <div className="space-y-2">
                  <p className="text-xs text-gray-500 mb-1">Choose your nearest collection point — <span className="font-medium text-green-600">FREE</span></p>
                  {COLLECTION_POINTS.map((pt) => (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        <div className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${selectedPoint === pt.id ? "border-[#010f1c] bg-[#010f1c]" : "border-gray-300"}`}>
                          {selectedPoint === pt.id && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                        </div>
                        ...
                        <Package className={`w-5 h-5 flex-shrink-0 mt-0.5 ${selectedPoint === pt.id ? "text-[#010f1c]" : "text-gray-300"}`} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Payment method */}
              <div>
                <p className="text-sm font-semibold text-gray-700 mb-2">How will you pay?</p>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    >
                      ...
                      <span className="text-xs font-medium text-gray-700 text-center leading-tight">{opt.label}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="px-6 py-3 bg-gray-50 space-y-1">
            {discountAmount > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Coupon <span className="font-medium text-gray-700">{storedCouponCode}</span></span>
                <span className="text-green-600 font-medium">−${discountAmount.toFixed(2)}</span>
              </div>
            )}
            {fulfillment === "delivery" && selectedAddress && (
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Delivery fee {isHarare ? "(estimate)" : ""}</span>
                <span className="text-gray-700 font-medium">
                  {isHarare ? `~$${estimatedDeliveryFee.toFixed(2)}` : feePrice ? `$${feePrice.toFixed(2)}` : "TBC"}
                </span>
              </div>
            )}
            <div className="flex justify-between font-bold text-base text-gray-900 pt-1">
              <span>Total {isHarare && fulfillment === "delivery" ? "(approx.)" : ""}</span>
              <span>
            ...
            {!sent ? (
              <>
                <button
                  ...
                  onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.backgroundColor = "#1db954")}
                  onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.backgroundColor = "#25D366")}
                >
                  <MessageCircle className="w-5 h-5" />
                  Place Order via WhatsApp
                </button>
                <button onClick={onClose} className="w-full text-sm text-gray-400 hover:text-gray-600 transition mt-3 py-1">
                  Cancel
                </button>
              </>
            ) : (
              <div className="flex flex-col items-center gap-3 py-2 text-center">
                ...
                <p className="text-base font-semibold text-gray-900">Order sent to WhatsApp!</p>
                ...
                <button onClick={onClose} className="mt-2 w-full py-3 rounded-xl bg-gray-100 hover:bg-gray-200 transition text-sm font-medium text-gray-700">
                  Done
                </button>
              </div>
            )}
...
const CartPage = () => {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm({
    defaultValues: { label: "Home", name: "", street: "", city: "", zip: "", country: "Zimbabwe", isDefault: "false" },
  });
  ...
  const { mutate: addAddress, isPending: isAddingAddress } = useMutation({
    ...
    onError: () => toast.error("Failed to add address. Please try again."),
  });
  ...
  const couponCodeApplyHandler = async () => {
    ...
    try {
      ...
      if (res.data.valid) {
        ...
        setDiscountAmount(0); setDiscountPercent(0); setDiscountedProductId("");
        setCouponError(res.data.message || "Coupon not valid for any items in cart.");
      }
    } catch (err: any) {
      setDiscountAmount(0); setDiscountPercent(0); setDiscountedProductId("");
  ...
  const removeItem = (id: string) => removeFromCart(id, user, location, deviceInfo);
  ...
  return (
    <div className="w-full bg-white">
      <div className="md:w-[80%] w-[95%] mx-auto min-h-screen">
        ...
        {cart.length === 0 ? (
          <div className="text-center text-gray-600 text-lg py-20">Your cart is empty! Start adding products.</div>
        ) : (
                  ...
                  <tr key={item.id} className="border-b border-b-[#0000000e]">
                    ...
                    <td>
                      <div className="flex justify-center items-center border border-gray-200 rounded-[20px] w-[100px] mx-auto p-[6px]">
                        <button className="text-[#000] cursor-pointer text-xl" onClick={() => decreaseQuantity(item.id)}>-</button>
                        <span className="px-4">{item.quantity}</span>
                        <button className="text-[#000] cursor-pointer text-xl" onClick={() => increaseQuantity(item.id)}>+</button>
                      </div>
                    </td>
            ...
            <div className="p-6 shadow-md w-full lg:w-[30%] bg-[#f9f9f9] rounded-lg">
              ...
              <div className="mb-4">
                ...
                <div className="flex">
                  ...
                  />
                  <button className="bg-blue-500 cursor-pointer text-white px-4 rounded-r-md hover:bg-blue-600 transition-all" onClick={couponCodeApplyHandler}>
                    Apply
                  </button>
                </div>
                {couponError && <p className="text-sm pt-2 text-red-500">{couponError}</p>}
              </div>
              <hr className="my-4 border-slate-200" />
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center mb-[7px]">
                  ...
                  <button onClick={() => setShowAddressModal(true)} className="flex items-center gap-1 text-xs text-blue-600 font-medium hover:underline">
                ...
                {addresses.length > 0 ? (
                  ...
                  <p className="text-sm text-slate-800">
                    No saved addresses.{" "}
                    <button onClick={() => setShowAddressModal(true)} className="text-blue-600 hover:underline font-medium">Add one now</button>
                  </p>
                )}
              </div>
              <hr className="my-4 border-slate-200" />
              ...
              <div className="mb-4 flex items-start gap-2 bg-gray-50 border border-gray-200 rounded-xl p-3">
                ...
                <p className="text-xs text-gray-600">
                  Prefer to collect? We have <span className="font-semibold text-gray-800">2 shops in Harare CBD</span>. Choose collection at checkout.
                </p>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 cursor-pointer mt-4 py-3 rounded-lg font-semibold text-white transition-all active:scale-[0.98]"
                style={{ backgroundColor: "#25D366" }}
                onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.backgroundColor = "#1db954")}
                onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.backgroundColor = "#25D366")}
              >
              ...
              <p className="text-xs text-center text-gray-400 mt-2">We'll confirm your order on WhatsApp</p>
            </div>
          </div>
        )}
      </div>
      ...
      {showAddressModal && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
          <div className="bg-white w-full max-w-md p-6 rounded-md shadow-md relative">
            <button className="absolute top-3 right-3 text-gray-500 hover:text-gray-800" onClick={() => { setShowAddressModal(false); reset(); }}>
              <X className="w-5 h-5" />
            </button>
            <h3 className="text-lg font-semibold mb-4 text-gray-800">Add New Address</h3>
            <form onSubmit={handleSubmit((data) => addAddress({ ...data, isDefault: data.isDefault === "true" }))} className="space-y-3">
              <select {...register("label")} className="form-input"><option value="Home">Home</option><option value="Work">Work</option><option value="Other">Other</option></select>
              ...
              <button type="submit" disabled={isAddingAddress} className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white text-sm py-2 rounded-md hover:bg-blue-700 transition disabled:opacity-70">
                {isAddingAddress && <Loader2 className="animate-spin w-4 h-4" />}
