...
const Page = () => {
  ...
  useEffect(() => {
    ...
    // Handle single category from header nav (e.g. ?category=Electronics)
    if (categoryParam) {
    ...
    // Handle subcategory from header nav (e.g. ?subcategory=Phones)
    if (subcategoryParam) {
      setSelectedSubcategories([subcategoryParam]);
    }

    if (colorsParam) setSelectedColors(colorsParam.split(","));
    if (sizesParam) setSelectedSizes(sizesParam.split(","));
  ...
  // Only fetch after URL params have been read and state initialized
  useEffect(() => {
    if (!initialized) return;
    updateURL();
    fetchFilteredProducts();
  }, [priceRange, selectedCategories, selectedSubcategories, selectedColors, selectedSizes, page, initialized]);
  ...
  const toggleCategory = (label: string) => {
    ...
    // Clear subcategories when changing parent category
                ...
                data?.categories?.map((category: any) => {
                  ...
                  return (
                    <li key={category}>
                      ...
                      {/* Show subcategories inline when parent is selected */}
                      {isChecked && hasSub && (
                        <ul className="ml-6 mt-1 space-y-1">
                          {data.subCategories[category].map((sub: string) => (
                            <li key={sub}>
